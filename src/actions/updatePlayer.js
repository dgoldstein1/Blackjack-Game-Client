...
export const updatePlayer = action => (dispatch, getState) => {
  ...
  const myIndex = playersInGame.findIndex(
    player => player.id === userLogedIn.id
  );
  ...
  if (score > 21) {
    ...
    if (playersInGame[myIndex].score > playersInGame[otherPlayerIndex].score) {
      ...
      playersInGame[myIndex].score === playersInGame[otherPlayerIndex].score
    ) {
  ...
  if (action === "call") {
    ...
    return request
      ...
      .then(request.get(`${baseUrl}/game`).then())
      .catch(console.error);
  }
